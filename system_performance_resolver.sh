...
kill_resource_hogs() {
    ...
    local top_cpu_pids
    local top_cpu_pids=$(ps aux --sort=-%cpu --no-headers | head -5 | awk '$3 > 10.0 {print $2 ":" $11}')
...
interact_with_htop() {
    ...
    local htop_pid
    local htop_pid=$(pgrep htop || echo "")
    ...
    if [[ -n "$htop_pid" ]]; then
        ...
        local htop_window
    local htop_window=$(xdotool search --name "htop" 2>/dev/null | head -1 || echo "")
...
optimize_system() {
    ...
    local zombies
    local zombies=$(ps aux | awk '$8 ~ /^Z/ {print $2}' || echo "")
    if [[ -n "$zombies" ]]; then
...
fix_vscode_config() {
    ...
    log_info "📝 MCP server should use: docker run -i --rm -e MCP_MODE=stdio ghcr.io/czlonkowski/n8n-mcp:sha-df03d42"
...
monitor_recovery() {
    ...
    local new_load
    local new_load=$(cat /proc/loadavg | cut -d' ' -f1)
    log_info "📈 New load average: $new_load"
    
    # Check memory
    local new_mem
    local new_mem=$(free -h | grep "Mem:" | awk '{print $4}')
    log_info "🧠 Available memory: $new_mem"
    
    # Check for remaining problematic processes
    local remaining_hogs
    local remaining_hogs=$(ps aux --sort=-%cpu --no-headers | head -3 | awk '$3 > 20.0 {print $2 ":" $11}')
